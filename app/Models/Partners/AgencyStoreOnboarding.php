...
/**
 * `agency_store_onboarding` — one row per "Add store" attempt.
 * `state_token` stores a SHA-256 hash only; the raw token is handed to
 * the browser once (in the wait-page URL) and never persisted.
 */
class AgencyStoreOnboarding extends Model
{
    ...
    {
        return $this->belongsTo(\App\Models\Store::class, 'created_store_id');
    }
