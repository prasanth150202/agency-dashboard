    ...
    <x-modal name="add-store" :open-on-load="$errors->store->any()">
        <div class="flex items-start justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-ink-500">Connect a new client store to {{ $organisation->name }}.</p>
            </div>
        ...
        <form method="POST" action="{{ route('stores.store') }}" class="mt-5 space-y-4">
            @csrf

            <div>
                <label for="store-name" class="mb-1.5 block text-sm font-medium text-ink-700">Store name</label>
                <input
                    id="store-name"
                    type="text"
                    name="name"
                    value="{{ old('name') }}"
                    placeholder="e.g. Lushra"
            ...
            <div>
                <label for="store-domain" class="mb-1.5 block text-sm font-medium text-ink-700">Shopify domain</label>
                <input
                    id="store-domain"
                    type="text"
                    name="shop_domain"
                    value="{{ old('shop_domain') }}"
                    placeholder="lushra.myshopify.com"
                    class="w-full rounded-lg border border-ink-200 px-3 py-2 text-sm text-ink-900 placeholder-ink-400 outline-none focus:border-brix-400 focus:ring-2 focus:ring-brix-100"
                >
            ...
            <div>
                <label for="store-admin-url" class="mb-1.5 block text-sm font-medium text-ink-700">Admin URL <span class="font-normal text-ink-400">(optional)</span></label>
                <input
                    id="store-admin-url"
                    type="text"
                    name="admin_url"
                    value="{{ old('admin_url') }}"
                    placeholder="https://lushra.myshopify.com/admin"
                    class="w-full rounded-lg border border-ink-200 px-3 py-2 text-sm text-ink-900 placeholder-ink-400 outline-none focus:border-brix-400 focus:ring-2 focus:ring-brix-100"
                >
                @error('admin_url', 'store')
                    <p class="mt-1.5 text-xs text-rose-600">{{ $message }}</p>
                @enderror
            </div>
