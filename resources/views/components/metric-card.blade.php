@props(['label', 'value', 'delta' => null, 'icon' => null, 'caption' => 'vs last period'])

<div class="rounded-2xl border border-ink-200/70 bg-white p-5 shadow-subtle">
    <div class="flex items-center justify-between">
        <span class="text-sm font-medium text-ink-500">{{ $label }}</span>
        @if ($icon)
            <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-brix-50 text-brix-600">
                <x-dynamic-component :component="'lucide-' . $icon" class="h-4 w-4" />
            </span>
        @endif
    </div>

    <p class="mt-3 text-2xl font-semibold tracking-tight text-ink-900">{{ $value }}</p>

    @if (! is_null($delta))
        @php
            $positive = $delta >= 0;
        @endphp
        <p class="mt-2 inline-flex items-center gap-1 text-xs font-medium {{ $positive ? 'text-emerald-600' : 'text-rose-600' }}">
            <x-lucide-trending-up class="h-3.5 w-3.5 {{ $positive ? '' : 'rotate-180' }}" />
            {{ $positive ? '+' : '' }}{{ $delta }}%
            <span class="font-normal text-ink-400">{{ $caption }}</span>
        </p>
    @endif
</div>
